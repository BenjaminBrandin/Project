...
# def pathMinkowskiSumVertices(listOfFormulas : List[StlTask], edgeList: List[(int,int)]) -> ca.MX  :
#     """
...
# def computeNewTaskGraph(task_graph:nx.Graph, comm_graph:nx.Graph, task_edges:List[Tuple], comm_info:Dict[str, Dict], problemDimension = 2, maxInterRobotDistance = 3)-> nx.Graph: 
...
#     for task_name, task_dict in enumerate(comm_info.items()):

#         edge = task_dict["EDGE"]
#         edge_container: EdgeTaskContainer = task_graph[edge[0]][edge[1]]["container"] # graph[edge[0]][edge[1]]["container"] is an instance of EdgeTaskContainer and have a .task_list
...
#                     subformula = StlTask(predicate=PredicateFunction(), temporal_operator=originalTemporalOperator)
...
#                     globalOptimizer.set_initial(subformula.centerVar , task_graph.nodes[targetNode]["pos"]-task_graph.nodes[sourceNode]["pos"]) 
...
#             for sourceNode,targetNode in   edgesThroughPath :
#                     graph.edges[sourceNode,targetNode]["edgeObj"].flagOptimizationInvolvement()
